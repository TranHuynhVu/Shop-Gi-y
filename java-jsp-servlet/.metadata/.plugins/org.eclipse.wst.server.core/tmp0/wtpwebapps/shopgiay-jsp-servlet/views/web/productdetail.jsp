			...
			<div class="row">
						...
						<div class="wrap-slick3 flex-sb flex-w">
							...
							<div class="slick3 gallery-lb">
								<c:forEach var="image" items="${images}">
									...
									<div class="item-slick3" id="imageDataThumb1"
										data-thumb="${image.getID()}">
										<div class="wrap-pic-w pos-relative">
											<img id="imageIMG1" src="${image.getID()}" alt="IMG-PRODUCT">
											<a id="imageA1"
												class="flex-c-m size-108 how-pos1 bor0 fs-16 cl10 bg0 hov-btn3 trans-04"
												href="${image.getID()}"> <i class="fa fa-expand"></i>
											</a>
										</div>
									</div>
				...
				<div class="col-md-6 col-lg-5 p-b-30">
					<div class="p-r-50 p-t-5 p-lr-0-lg click-js-addcart-detail">
						<h4 class="mtext-105 cl2 js-name-detail p-b-14" id="nameShoe">${detall_shoes.getNAME()}</h4>

						<span class="mtext-106 cl2" id="PRICE">
							$${detall_shoes.getPRICE()} </span>
						...
						<div class="p-t-33">
							<div class="flex-w flex-r-m p-b-10">
								...
								<div class="size-204 respon6-next">
									<div class="rs1-select2 bor8 bg0">
										<select class="js-select2" name="sizes" id="sizes">
											<option>Choose an option</option>
											<c:forEach var="sizes" items="${size}">
												<option>${sizes.getSIZE()}</option>
											</c:forEach>
										</select>
								...
								<div class="size-204 respon6-next">
									<div class="rs1-select2 bor8 bg0">
										<select class="js-select2" name="color" id="color">
											<option>Choose an option</option>
											<c:forEach var="sizes" items="${colors}">
												<option>${sizes.getNAMEE()}</option>
											</c:forEach>
										</select>
										<div class="dropDownSelect2"></div>
									</div>
							...
							<div class="flex-w flex-r-m p-b-10">
								<div class="size-204 flex-w flex-m respon6-next">
									...
									<input type="hidden" name="iddf" id="iddf" value="${iddf}">
									<input type="hidden" name="id_Shoe" id="id_Shoe" value="">
									...
									<div class="p-b-30 m-lr-15-sm">
										...
										<c:forEach var="comment" items="${comments}">
											<div class="flex-w flex-t p-b-68">
												<div class="wrap-pic-s size-109 bor0 of-hidden m-r-18 m-t-6">
													<img
														src="<c:url value="/template/web/images/avatar-01.jpg"/>"
														alt="AVATAR">
												</div>
												...
												<div class="size-207">
													<div class="flex-w flex-sb-m p-b-17">
														<span class="mtext-107 cl2 p-r-20">
															${comment.getNAMEE()} </span> <span class="fs-18 cl11"> <c:forEach
																var="i" begin="1" end="${comment.getRATE()}">
													...
													<p class="stext-102 cl6">${comment.getDETAILCR()}</p>
												</div>
											</div>
										</c:forEach>
										...
										<div class="flex-w flex-m p-t-50 p-b-23">
											<span class="stext-102 cl3 m-r-16 "> Your
												Rating </span> <span class="wrap-rating fs-18 cl11 pointer starComment">
												<i class="item-rating pointer zmdi zmdi-star-outline"></i> <i
										...
										<input type="hidden" name="iddetailshoe" id="iddetailshoe" value="${iddf}">
										<input type="hidden" name="idaccount" id="idaccount" value="${idacc}">
										<button
